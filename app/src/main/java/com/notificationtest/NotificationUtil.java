...
public class NotificationUtil {
    ...
    public void postBarNotification() {
        /*Intent intent = new Intent(context, MainActivity.class);  //需要跳转指定的页面
        PendingIntent pendingIntent = PendingIntent.getActivity(context, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT);*/
    ...
    public void postBigTextNotification() {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN) {
            Notification.BigTextStyle textStyle = null;
            textStyle = new Notification.BigTextStyle();
     ...
     */
    public void postBigPictureNotification() throws Exception {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN) {
    ...
    public void postCustomNotification() {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN) {
            RemoteViews contentViews = new RemoteViews(context.getPackageName(), R.layout.mynotification);
            contentViews.setImageViewResource(R.id.imageNotifi, R.drawable.ic_stop);
            /*contentViews.setTextViewText(R.id.titleTV, "标题");
            contentViews.setTextViewText(R.id.textTV, "通知内容");*/
            ...
            //设置点击的事件
            Intent buttonIntent = new Intent(context, buttonBrodcast.class);
            buttonIntent.putExtra("btn", "read");
            PendingIntent intent_read = PendingIntent.getBroadcast(context, 0, buttonIntent, PendingIntent.FLAG_UPDATE_CURRENT);
            contentViews.setOnClickPendingIntent(R.id.read, intent_read);

            Intent buttonIntent1 = new Intent(context, deleteBrodcast.class);
            buttonIntent1.putExtra("btn", "delete");
            PendingIntent intent_delete = PendingIntent.getBroadcast(context, 0, buttonIntent1, PendingIntent.FLAG_UPDATE_CURRENT);
            contentViews.setOnClickPendingIntent(R.id.delete, intent_delete);
            ...
            foregroundNote.bigContentView = contentViews;

            notificationManager.notify(0, foregroundNote);
        }
    }

    private boolean vsionC() {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN)
            return true;
        else {
            Toast.makeText(context, "当前版本不支持", Toast.LENGTH_SHORT);
            return false;
        }
    }
